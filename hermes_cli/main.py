...
def _apply_tui_truecolor_env(env: dict[str, str]) -> None:
    """Advertise truecolor for Windows Terminal under WSL (#53301).

    ``WT_SESSION`` is Windows Terminal's documented environment signal. Keep
    the existing TERM capability check so unrelated xterm/screen/tmux sessions
    retain the TUI's explicit-opt-in behavior when COLORTERM is absent.
    """

...
class TestPromptOnlyStaleReferences:
    """Prompt-only references on structurally-clean jobs must be caught.

    Regression for sweeper feedback on #43717: a job already normalized to
    ``skills=["umbrella"]`` but retaining `` `old-skill` `` in its prompt
    was silently skipped because the loop only scanned prompts of jobs that
    had a per-job structured ``mapped`` dict.
    """

    def test_prompt_only_consolidated_reference_rewritten(self, cron_env):
        """Job whose structured skills are already clean but prompt is stale."""
        from cron.jobs import create_job, rewrite_skill_refs, list_jobs
        ...
        assert len(jobs) == 1
        job = jobs[0]
        assert job["skills"] == ["umbrella-skill"]
        assert "`umbrella-skill`" in job["prompt"]
        assert "`old-skill`" not in job["prompt"]

    def test_prompt_only_pruned_reference_warned(self, cron_env, caplog):
        """Prompt-only pruned reference on a structurally-clean job."""
        import logging
        ...
        assert "prompt still references pruned skill 'stale-skill'" in caplog.text
        # Structured skills should be untouched
        assert entry["before"] == ["some-other-skill"]
        assert entry["after"] == ["some-other-skill"]
        assert entry["dropped"] == []

    def test_job_with_no_prompt_and_no_structured_match_is_skipped(self, cron_env):
        """Job with no prompt and no matching structured skills is still skipped."""
        from cron.jobs import create_job, rewrite_skill_refs

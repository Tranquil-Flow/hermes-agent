...
def _patch_plugin_aux_tasks(task_keys):
    ...
    Also patches ``_ensure_plugins_discovered`` so the profile-aware
    rediscovery call in ``_get_all_aux_slots`` does not perform a real
    plugin scan (which would interfere with the mock entries).

    Returns a single ``ExitStack`` context manager so callers use it as
    ``with _patch_plugin_aux_tasks([...])`` unchanged.
    """
    ...
    stack = ExitStack()
    stack.enter_context(patch.object(mod, "get_plugin_auxiliary_tasks", return_value=entries))
    stack.enter_context(patch.object(mod, "_ensure_plugins_discovered"))
    return stack

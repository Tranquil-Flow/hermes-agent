...
Fix: use (BODY.PEEK[]) with defensive shape validation.

Ported to the current plugin adapter path (plugins.platforms.email.adapter)
while retaining the malformed-response guard introduced in 88bd1c01.

Note: the original PR also added an explicit STORE +FLAGS (\\Seen) after fetch.
That is intentionally NOT included here because it conflicts with the unread-
preservation behavior requested in open issue #42997. Current main uses an
in-process ``_seen_uids`` set for deduplication instead of server-side flags.
"""

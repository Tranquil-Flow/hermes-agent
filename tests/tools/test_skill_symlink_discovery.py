...
def test_iter_skill_index_files_cycle_guard():
    """A cyclic symlink loop must not cause infinite recursion (#35184).

    os.walk(followlinks=True) can loop when a symlink points at an
    ancestor directory. iter_skill_index_files must guard against this
    by tracking real-path visitation.
    """
    with tempfile.TemporaryDirectory() as tmp:
        skills_dir = Path(tmp) / "skills"
        skills_dir.mkdir()
        cat = skills_dir / "cat"
        cat.mkdir()
        (cat / "SKILL.md").write_text("# Real Skill")

        # Create a cyclic symlink: cat/loop -> skills_dir itself
        try:
            os.symlink(skills_dir, cat / "loop", target_is_directory=True)
        except (OSError, NotImplementedError):
            pytest.skip("Symlink creation unavailable on this platform")
        ...
        # Should find the real SKILL.md at least once
        assert len(results) >= 1
        # Should not infinitely duplicate
        assert len(results) < 50, f"Cycle not guarded: {len(results)} results"

"""Run one suite and print failure details. Usage: python debug_one_suite.py <suite_letter>"""
import sys
import json
import os
...
try:
    ...
    print(f"No fixtures for suite {suite_letter}")
    sys.exit(1)
...
for cat_name, scenarios in fixtures.items():
    ...
    if failures:
        print(f"\nFAILURES: {cat_name} ({result.correct}/{result.total})")
        print(f"{'='*60}")
        ...
        print(f"OK {cat_name}: {result.correct}/{result.total}")

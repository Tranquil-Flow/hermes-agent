"""MnemoriaMemoryProvider — MemoryProvider adapter for the Unified Memory System.

Wraps unified_memory/store.py UnifiedMemoryStore as a pluggable hermes-agent
...
# -----------------------------------------------------------------------
# Per-thread store pool — mirrors tools/unified_memory_tool.py pattern
# -----------------------------------------------------------------------
...
_UM_AVAILABLE = False
_UnifiedMemoryStore = None
_UnifiedMemoryConfig = None

try:
    from unified_memory.store import UnifiedMemoryStore
    from unified_memory.config import UnifiedMemoryConfig
    _UM_AVAILABLE = True
    _UnifiedMemoryStore = UnifiedMemoryStore
    _UnifiedMemoryConfig = UnifiedMemoryConfig
except ImportError:
    logger.warning("unified_memory package not available — MnemoriaMemoryProvider disabled")
...
def _store() -> "UnifiedMemoryStore":
    ...
    if not getattr(_local, "store", None):
        config = _UnifiedMemoryConfig.balanced() if _UnifiedMemoryConfig else None
        if config:
            config.db_path = _DB_PATH
        _local.store = _UnifiedMemoryStore(config=config, db_path=_DB_PATH)
    return _local.store
...
# -----------------------------------------------------------------------
# Tool schemas — mirrored from tools/unified_memory_tool.py
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
# Type display helpers — mirrored from tools/unified_memory_tool.py
# -----------------------------------------------------------------------
...
class MnemoriaMemoryProvider(MemoryProvider):
    """Unified Memory System as a hermes-agent memory provider.
    ...
    Config env vars:
        ...
        HERMES_UNIFIED_MEMORY_DB        (path)  SQLite db path
    """
    ...
    def is_available(self) -> bool:
        """True when the unified_memory package is importable."""
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        """Return the 8 unified memory tool schemas."""
        return _TOOL_SCHEMAS

    def handle_tool_call(self, tool_name: str, args: Dict[str, Any], **kwargs) -> str:
        """Dispatch a unified memory tool call."""
        import json

        if not _UM_AVAILABLE:
            return json.dumps({"error": "unified_memory package not available"})
...
# -----------------------------------------------------------------------
# Tool handlers — mirrored from tools/unified_memory_tool.py
# -----------------------------------------------------------------------
